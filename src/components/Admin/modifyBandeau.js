...
if (store.getState().Tennis.logged === true) {
  ...
  const [file, setFile] = useState(undefined);
      ...
      useEffect(() => {
        setLoading(true);
        fetch(`https://backtennis.herokuapp.com/bandeau/${idArt}`)
      ...
      const modifyBandeau = (data, event) => {
  ...
  return (
   ...
   <div className="modBandeau" id="modifyBandeau" key={idArt}>
    <div><h3>Modifier un item du Bandeau</h3></div>
              <form ref={form} id="formmodify" className="form" onSubmit={handleSubmit(modifyBandeau)}>
              <span ><button type="button" className="closeButton" onClick={closeBandeau}>X</button></span>
        ...
        {img === null ? (
        <div style={{ backgroundImage: `url(${bandeau.image})`, backgroundSize: 'cover',
        backgroundRepeat: 'no-repeat', width: '100%', height: '17rem', backgroundPositionX: 'center',
        backgroundPositionY: 'center' }}></div> 
        ) : (
 ...
 <select name="imagebandeau" className="selectImg" id="bandeau-select" onChange={(e) => picChange(e)}>
  <option>Choisir une image</option>
        ...
        onInit={(evt, editor) => editorRef.current = editor}
          ...
          name="contenu"
        />
        <button className="send_button" type='submit'> Envoyer </button>
        </form>
      </div>
    ) : ('TINTIN')}
      </>
